...
@ActiveProfiles("test")
@DisplayName("AuthApiController - tests extendidos")
class AuthApiControllerExtendedTest {
    ...
    void apiMe_conTokenAdmin_retornaInfo() throws Exception {
        mockMvc.perform(get("/api/me").header("Authorization", "Bearer " + tokenAdmin))
    ...
    void apiMe_conTokenChef_retornaInfo() throws Exception {
        mockMvc.perform(get("/api/me").header("Authorization", "Bearer " + tokenChef))
                .andExpect(status().isOk())
    ...
    void apiMe_conTokenUsuario_retornaInfo() throws Exception {
        mockMvc.perform(get("/api/me").header("Authorization", "Bearer " + tokenUsuario))
                .andExpect(status().isOk())
    ...
    void apiMe_conTokenInvalido_retornaForbidden() throws Exception {
        mockMvc.perform(get("/api/me").header("Authorization", "Bearer token.invalido.jwt"))
                .andExpect(status().isForbidden());
    }

    @Test
    @DisplayName("Login con usuario chef retorna accessToken y roles")
    void login_conChef_retornaToken() throws Exception {
        LoginRequest req = new LoginRequest("chef", "chef123");
    ...
    @Test
    @DisplayName("Login con usuario normal retorna accessToken válido")
    void login_conUsuario_retornaToken() throws Exception {
        LoginRequest req = new LoginRequest("usuario", "usuario123");
        mockMvc.perform(post("/api/auth/login")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isOk())
    ...
    void apiRecetaById_existente_retornaReceta() throws Exception {
        mockMvc.perform(get("/api/recetas/1").header("Authorization", "Bearer " + tokenAdmin))
                .andExpect(status().isOk())
