...
class InteraccionServiceSubirMedioTest {
    ...
    @Mock private MedioRecetaRepository medioRepository;
    @Mock private RecetaRepository      recetaRepository;

    @InjectMocks private InteraccionService interaccionService;
    ...
    void subirMedio_conImagenJpg_guardaFoto() throws IOException {
        MockMultipartFile archivo = new MockMultipartFile("file", "foto.jpg", "image/jpeg", "contenido".getBytes());
        when(recetaRepository.findById(1L)).thenReturn(Optional.of(recetaMock));
        interaccionService.subirMedio(1L, archivo, "usuario");
        verify(medioRepository).save(argThat(m -> "FOTO".equals(m.getTipo()) && m.getNombreArchivo().endsWith(".jpg")));
    }
    ...
    void subirMedio_conImagenPng_guardaFoto() throws IOException {
        MockMultipartFile archivo = new MockMultipartFile("file", "imagen.png", "image/png", "datos".getBytes());
        when(recetaRepository.findById(1L)).thenReturn(Optional.of(recetaMock));
    ...
    void subirMedio_conVideoMp4_guardaVideo() throws IOException {
        MockMultipartFile archivo = new MockMultipartFile("file", "video.mp4", "video/mp4", "datos".getBytes());
        when(recetaRepository.findById(1L)).thenReturn(Optional.of(recetaMock));
        interaccionService.subirMedio(1L, archivo, "chef");
        verify(medioRepository).save(argThat(m -> "VIDEO".equals(m.getTipo()) && m.getNombreArchivo().endsWith(".mp4")));
    }
    ...
    void subirMedio_conVideoMov_guardaVideo() throws IOException {
        MockMultipartFile archivo = new MockMultipartFile("file", "clip.mov", "video/quicktime", "datos".getBytes());
        when(recetaRepository.findById(1L)).thenReturn(Optional.of(recetaMock));
        interaccionService.subirMedio(1L, archivo, "chef");
    ...
    void subirMedio_asignaUsuarioCorrecto() throws IOException {
        MockMultipartFile archivo = new MockMultipartFile("file", "foto.jpg", "image/jpeg", "datos".getBytes());
        when(recetaRepository.findById(1L)).thenReturn(Optional.of(recetaMock));
    ...
    void subirMedio_generaNombreUUID() throws IOException {
        MockMultipartFile archivo = new MockMultipartFile("file", "foto_original.jpg", "image/jpeg", "datos".getBytes());
        when(recetaRepository.findById(1L)).thenReturn(Optional.of(recetaMock));
        interaccionService.subirMedio(1L, archivo, "usuario");
        verify(medioRepository).save(argThat(m -> !m.getNombreArchivo().contains("foto_original")));
    }
    ...
    void subirMedio_archivoVacio_lanzaExcepcion() {
        ...
        assertThrows(IllegalArgumentException.class, () -> interaccionService.subirMedio(1L, archivo, "usuario"));
    ...
    void subirMedio_extensionNoPermitida_lanzaExcepcion() {
        MockMultipartFile archivo = new MockMultipartFile("file", "script.exe", "application/octet-stream", "datos".getBytes());
        assertThrows(IllegalArgumentException.class, () -> interaccionService.subirMedio(1L, archivo, "usuario"));
        verify(medioRepository, never()).save(any());
    }
    ...
    void subirMedio_recetaNoExiste_lanzaExcepcion() {
        ...
        assertThrows(IllegalArgumentException.class, () -> interaccionService.subirMedio(999L, archivo, "usuario"));
    }
    ...
    void subirMedio_recetaIdNull_lanzaExcepcion() {
        MockMultipartFile archivo = new MockMultipartFile("file", "foto.jpg", "image/jpeg", "datos".getBytes());
        assertThrows(NullPointerException.class, () -> interaccionService.subirMedio(null, archivo, "usuario"));
    }

    @Test
    @DisplayName("subirMedio con archivo WebP guarda como FOTO")
    void subirMedio_conWebp_guardaFoto() throws IOException {
        MockMultipartFile archivo = new MockMultipartFile("file", "imagen.webp", "image/webp", "datos".getBytes());
        when(recetaRepository.findById(1L)).thenReturn(Optional.of(recetaMock));
        interaccionService.subirMedio(1L, archivo, "usuario");
    ...
    void subirMedio_conAvi_guardaVideo() throws IOException {
        MockMultipartFile archivo = new MockMultipartFile("file", "clip.avi", "video/x-msvideo", "datos".getBytes());
        when(recetaRepository.findById(1L)).thenReturn(Optional.of(recetaMock));
        interaccionService.subirMedio(1L, archivo, "usuario");
